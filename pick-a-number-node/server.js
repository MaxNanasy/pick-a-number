...
var db = new sqlite3.Database('db', function (error) {
  if (error) throw error;
  db.run('create table if not exists Game (id guid primary key not null, number integer not null)', function (error) {
    if (error) throw error;
    db.run('create table if not exists Guess (gameId guid not null, indexNumber integer not null, number integer not null, primary key (gameId, indexNumber));', function (error) {
    if (error) throw error;
